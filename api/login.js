...
// Har bir muvaffaqiyatli login uchun user_sessions'ga bir qator yozadi (server-side —
// endi login shu yerda tekshiriladi, shuning uchun sessiya yozuvi ham shu yerda,
// mijoz o'zini "men shu userman" deb da'vo qilishiga tayanmaydi). Xato bo'lsa ham
// loginni to'xtatmaydi — faqat sessionId null qaytadi (heartbeat/logout ishlamaydi,
// lekin foydalanuvchi normal kira oladi).
async function logSession(user) {
    if (!SUPABASE_URL || !SUPABASE_KEY) return null;
    try {
        const r = await fetch(`${SUPABASE_URL}/rest/v1/user_sessions`, {
            method: 'POST',
            headers: {
                apikey: SUPABASE_KEY,
            ...
            body: JSON.stringify({ user_id: String(user.id), user_name: user.full_name || user.username, user_role: user.role })
        });
        if (!r.ok) return null;
        const rows = await r.json();
        return Array.isArray(rows) && rows[0] ? rows[0].id : null;
    } catch (e) {
        console.warn('[LOGIN] user_sessions yozishda xato (loginga ta\'sir qilmaydi):', e);
        return null;
    }
}
...
async function respondWithSession(res, user) {
    ...
    return res.status(200).json({ ok: true, user, sessionId });
}
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (unameUpper === 'AC1' && pwd === '123') {
            return respondWithSession(res, { id: 'AC1', username: 'AC1', role: 'ac_manager', full_name: "Ishlab Chiqarish Boshlig'i" });
        }
        if ((uname === '7007' && pwd === '1234') || (uname === '8008' && pwd === '1234')) {
            return respondWithSession(res, { id: uname, username: uname, role: 'stanok', full_name: uname === '7007' ? 'Jaloliddin R.' : 'Sardorbek M.' });
        }
        if (['kraska1', 'kraska2', 'kraska3'].includes(unameLower) && pwd === '123') {
            ...
            return respondWithSession(res, { id, username: uname, role: 'kraska', full_name: fn });
        }
        if (['Q1', 'QADOQ1'].includes(unameUpper) && pwd === '123') {
            return respondWithSession(res, { id: 'Q1', username: 'Q1', role: 'qadoqlash', full_name: 'Qadoqlovchi 1' });
        }
        ...
        if (foundUser) {
            return respondWithSession(res, { id: foundUser.id, username: foundUser.username, role: foundUser.role, full_name: foundUser.full_name });
        }
        ...
        if (foundEmp && (effectivePassword === foundEmp.id || effectivePassword === '123456')) {
            return respondWithSession(res, { id: foundEmp.id, username: foundEmp.full_name, role: 'employee', full_name: foundEmp.full_name, avatar_url: foundEmp.avatar_url });
        }
            ...
            try {
                ...
                if (sysRes.ok) {
                    ...
                    if (sysData && sysData.length > 0) {
                        const u = sysData[0];
                        return respondWithSession(res, { id: u.id || 'sys', username: u.username, role: u.role, full_name: u.full_name });
                    }
                }
                ...
                if (empRes.ok) {
                    ...
                    if (empData && empData.length > 0) {
                        ...
                        if (effectivePassword === emp.id || effectivePassword === '123456') {
                            return respondWithSession(res, { id: emp.id, username: emp.full_name, role: 'employee', full_name: emp.full_name, avatar_url: emp.avatar_url });
                        }
                    }
                }
