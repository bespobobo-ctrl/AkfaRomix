import { createClient } from '@supabase/supabase-js'

const supabaseUrl = import.meta.env.VITE_SUPABASE_URL || "https://dzsswblbpnjuluyqvewt.supabase.co";
const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImR6c3N3YmxicG5qdWx1eXF2ZXd0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc4OTI2NzcsImV4cCI6MjA5MzQ2ODY3N30.Kwgh1DIzb_j7AH2iEfI5LMboObXBaIm3SGk1JWF3LIk";

const realSupabase = createClient(supabaseUrl, supabaseAnonKey)

const makeImmediateReject = (message) => {
    const error = { message, code: 'OFFLINE', details: 'Database domain is paused' };
    const promise = Promise.resolve({ data: null, error });
    
    const builder = new Proxy(promise, {
        get(target, prop) {
            if (prop === 'then') {
                return function(onFulfilled, onRejected) {
                    return promise.then(onFulfilled, onRejected);
                };
            }
            return () => builder;
        }
    });
    return builder;
};

// Force offline proxy because the Supabase host domain is currently paused/NXDOMAIN
export const supabase = new Proxy(realSupabase, {
    get(target, prop) {
        if (prop === 'from') {
            return function(tableName) {
                return makeImmediateReject(`Supabase is offline (domain paused)`);
            };
        }
        if (prop === 'auth') {
            return {
                async signInWithPassword() {
                    return { data: { user: null }, error: { message: 'Database is offline' } };
                },
                async signOut() {}
            };
        }
        if (prop === 'storage') {
            return {
                from(bucketName) {
                    return {
                        async upload() { return { data: null, error: { message: 'Offline' } }; },
                        getPublicUrl() { return { data: { publicUrl: '' } }; }
                    };
                }
            };
        }
        return Reflect.get(target, prop);
    }
});

export function checkAuth(roles = []) {
    const user = JSON.parse(localStorage.getItem('currentUser'));
    if (!user) {
        window.location.href = '/index.html';
        return null;
    }
    if (roles.length > 0 && !roles.includes(user.role)) {
        window.location.href = '/index.html';
        return null;
    }
    return user;
}

export function logout() {
    localStorage.removeItem('currentUser');
    window.location.href = '/index.html';
}
